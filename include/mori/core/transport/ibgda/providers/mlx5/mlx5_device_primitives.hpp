...
/* ---------------------------------------------------------------------------------------------- */
template <>
__device__ uint64_t PostSend<ProviderType::MLX5>(IbgdaReadWriteReq& req) {
...
template <>
__device__ void PostRecv<ProviderType::MLX5>(IbgdaReadWriteReq& req) {
  uint32_t wqe_idx = req.qp_handle.post_idx & (req.qp_handle.wqe_num - 1);
  void* wqe_addr =
...
__device__ uint64_t PostReadWrite(IbgdaReadWriteReq& req, bool is_read) {
  ...
  return reinterpret_cast<uint64_t*>(req.qp_handle.queue_buff_addr)[0];
}

template <>
__device__ uint64_t PostWrite<ProviderType::MLX5>(IbgdaReadWriteReq& req) {
  return PostReadWrite(req, false);
}

template <>
__device__ uint64_t PostRead<ProviderType::MLX5>(IbgdaReadWriteReq& req) {
...
template <>
inline __device__ void UpdateCqDbrRecord<ProviderType::MLX5>(void* dbr_rec_addr,
                                                             uint32_t cons_idx) {
