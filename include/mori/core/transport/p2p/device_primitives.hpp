...
// TODO: too many repeated code, refactor
template <typename T>
...
template <>
inline __device__ void WarpAccum<hip_bfloat16>(hip_bfloat16* dest, hip_bfloat16** srcs,
                                               float* srcScales, size_t accumNum, size_t nelems) {
...
#pragma unroll
    for (int i = 0; i < accumNum; i++) {
      ...
      u.u32 = srcVals.x << 16;
      accumValFp32[0] += u.fp32 * srcScale;
      u.u32 = srcVals.x & 0xffff0000;
      accumValFp32[1] += u.fp32 * srcScale;

      u.u32 = srcVals.y << 16;
      accumValFp32[2] += u.fp32 * srcScale;
      u.u32 = srcVals.y & 0xffff0000;
      accumValFp32[3] += u.fp32 * srcScale;

      u.u32 = srcVals.z << 16;
      accumValFp32[4] += u.fp32 * srcScale;
      u.u32 = srcVals.z & 0xffff0000;
      accumValFp32[5] += u.fp32 * srcScale;

      u.u32 = srcVals.w << 16;
      accumValFp32[6] += u.fp32 * srcScale;
      u.u32 = srcVals.w & 0xffff0000;
      accumValFp32[7] += u.fp32 * srcScale;
    }

    uint4 accumVals;
    accumVals.x = (T(accumValFp32[0]).data | (T(accumValFp32[1]).data << 16));
    accumVals.y = (T(accumValFp32[2]).data | (T(accumValFp32[3]).data << 16));
    accumVals.z = (T(accumValFp32[4]).data | (T(accumValFp32[5]).data << 16));
    accumVals.w = (T(accumValFp32[6]).data | (T(accumValFp32[7]).data << 16));
...
template <typename T>
inline __device__ void WarpAccum(T* dest, T** srcs, size_t accumNum, size_t nelems) {
  constexpr int vecSize = 16 / sizeof(T);
  int laneId = threadIdx.x & (warpSize - 1);
  int offset = laneId * vecSize;
...
#pragma unroll
    for (int i = 0; i < accumNum; i++) {
      ...
      if constexpr (vecSize > 0) accumValFp32[0] += float(reinterpret_cast<T*>(&srcVals)[0]);
      if constexpr (vecSize > 1) accumValFp32[1] += float(reinterpret_cast<T*>(&srcVals)[1]);
      if constexpr (vecSize > 2) accumValFp32[2] += float(reinterpret_cast<T*>(&srcVals)[2]);
      if constexpr (vecSize > 3) accumValFp32[3] += float(reinterpret_cast<T*>(&srcVals)[3]);
      if constexpr (vecSize > 4) accumValFp32[4] += float(reinterpret_cast<T*>(&srcVals)[4]);
      if constexpr (vecSize > 5) accumValFp32[5] += float(reinterpret_cast<T*>(&srcVals)[5]);
      if constexpr (vecSize > 6) accumValFp32[6] += float(reinterpret_cast<T*>(&srcVals)[6]);
      if constexpr (vecSize > 7) accumValFp32[7] += float(reinterpret_cast<T*>(&srcVals)[7]);
      if constexpr (vecSize > 8) accumValFp32[8] += float(reinterpret_cast<T*>(&srcVals)[8]);
      if constexpr (vecSize > 9) accumValFp32[9] += float(reinterpret_cast<T*>(&srcVals)[9]);
      if constexpr (vecSize > 10) accumValFp32[10] += float(reinterpret_cast<T*>(&srcVals)[10]);
      if constexpr (vecSize > 11) accumValFp32[11] += float(reinterpret_cast<T*>(&srcVals)[11]);
      if constexpr (vecSize > 12) accumValFp32[12] += float(reinterpret_cast<T*>(&srcVals)[12]);
      if constexpr (vecSize > 13) accumValFp32[13] += float(reinterpret_cast<T*>(&srcVals)[13]);
      if constexpr (vecSize > 14) accumValFp32[14] += float(reinterpret_cast<T*>(&srcVals)[14]);
      if constexpr (vecSize > 15) accumValFp32[15] += float(reinterpret_cast<T*>(&srcVals)[15]);
    }
...
#pragma unroll
    for (int i = 0; i < accumNum; i++) {
      T* srcPtr = srcs[i];
      if (srcPtr == nullptr) continue;
      accumValFp32 += float(srcPtr[offset]);
    }
    dest[offset] = T(accumValFp32);
    offset += 1;
  }
...
#pragma unroll
    for (int i = 0; i < accumNum; i++) {
      ...
      u.u32 = srcVals.x << 16;
      accumValFp32[0] += u.fp32;
      u.u32 = srcVals.x & 0xffff0000;
      accumValFp32[1] += u.fp32;

      u.u32 = srcVals.y << 16;
      accumValFp32[2] += u.fp32;
      u.u32 = srcVals.y & 0xffff0000;
      accumValFp32[3] += u.fp32;

      u.u32 = srcVals.z << 16;
      accumValFp32[4] += u.fp32;
      u.u32 = srcVals.z & 0xffff0000;
      accumValFp32[5] += u.fp32;

      u.u32 = srcVals.w << 16;
      accumValFp32[6] += u.fp32;
      u.u32 = srcVals.w & 0xffff0000;
      accumValFp32[7] += u.fp32;
    }
