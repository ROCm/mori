...
static __device__ uint64_t PostSend(void* queue_buff_addr, uint32_t& post_idx, uint32_t wqe_num,
                                    uint32_t qpn, uintptr_t laddr, uint64_t lkey,
                                    size_t bytes_count);

template <ProviderType PrvdType>
static __device__ void PostRecv(void* queue_buff_addr, uint32_t wqe_num, uint32_t& post_idx,
                                uintptr_t laddr, uint64_t lkey, size_t bytes_count);

template <ProviderType PrvdType>
static __device__ uint64_t PostWrite(void* queue_buff_addr, uint32_t wqe_num, uint32_t& post_idx,
                                     uint32_t qpn, uintptr_t laddr, uint64_t lkey, uintptr_t raddr,
                                     uint64_t rkey, size_t bytes_count);

template <ProviderType PrvdType>
static __device__ uint64_t PostRead(void* queue_buff_addr, uint32_t wqe_num, uint32_t& post_idx,
                                    uint32_t qpn, uintptr_t laddr, uint64_t lkey, uintptr_t raddr,
                                    uint64_t rkey, size_t bytes_count);
