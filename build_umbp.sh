...
cmake --build . -j"$(nproc)" --target \
  umbp_master umbp_client umbp_pool_client \
  umbp_core \
  test_umbp_types test_umbp_client_registry test_umbp_block_index \
  ...
  test_umbp_local_block_index test_umbp_local_storage_manager test_umbp_local_client \
  test_umbp_local_follower_mode test_umbp_local_eviction
...
echo ""
echo "  Pool tests (distributed):  ${BUILD_DIR}/tests/cpp/umbp/pool/"
echo "  Local tests (single-node): ${BUILD_DIR}/tests/cpp/umbp/local/"
echo ""
