...
| `UMBP_DISTRIBUTED_MEDIUM` | `DRAM` | enum | The one medium this node serves: `DRAM`, `HBM` or `SSD`. Selecting `SSD` is the whole opt-in for a storage node — there is no separate pure-SSD flag. An unknown value is rejected at startup rather than silently defaulting. Standalone server binary only; the SGLang path uses the pybind `medium` field. |
