...
static void submit_single_io(umbp::SpdkIoRequest *req, void *bdev_desc) {
    spdk_bdev_io_completion_cb done = [](struct spdk_bdev_io *bio, bool ok,
        ...
        r->success = ok;
        if (ok && r->op == umbp::SpdkIoRequest::READ &&
...
void umbp_execute_io_cb(void *ctx) {
    ...
    submit_single_io(static_cast<umbp::SpdkIoRequest *>(ctx),
                     env.bdev_desc_);
}
...
void umbp_init_reactor_cb(void *arg1, void *arg2) {
    ...
    UMBP_LOG_INFO("SpdkEnv: reactor %d ready on core %u",
                  idx, spdk_env_get_current_core());
...
void umbp_app_start_cb(void *ctx) {
    ...
    if (rc != 0) {
        UMBP_LOG_ERROR("SpdkEnv: spdk_bdev_open_ext('%s') failed rc=%d",
                       cfg.bdev_name.c_str(), rc);
    ...
    UMBP_LOG_INFO("SpdkEnv: bdev '%s' opened — block_size=%u total_size=%lu",
                  cfg.bdev_name.c_str(), env->block_size_,
                  static_cast<unsigned long>(env->bdev_size_));
    ...
    for (int i = 1; i < env->num_reactors_; ++i) {
        struct spdk_event *ev = spdk_event_allocate(
            cores[i], umbp_init_reactor_cb, env,
...
void SpdkEnv::Shutdown() {
    ...
    for (int i = 0; i < num_reactors_; ++i) {
        struct spdk_event *ev = spdk_event_allocate(
            cores[i], umbp_cleanup_reactor_cb, this,
            reinterpret_cast<void *>(static_cast<intptr_t>(i)));
        spdk_event_call(ev);
    }
...
int SpdkEnv::SubmitIoBatchAsync(SpdkIoRequest **reqs, int count) {
    ...
    for (int r = 0; r < num_reactors_; ++r) {
        ...
        spdk_thread_send_msg(SPDK_THREAD(reactors_[r].spdk_thread),
                             umbp_execute_io_batch_cb, heads[r]);
    }
    return 0;
}
...
int SpdkEnv::DmaPoolAllocBatch(void **out_bufs, size_t needed, int count,
    ...
    for (int i = got; i < count; ++i) {
        ...
        if (!out_bufs[i]) {
            UMBP_LOG_ERROR("DmaPoolAllocBatch FAIL: malloc_fail_at=%d needed=%zu count=%d",
                           i, needed, count);
            return i;
        }
        ++got;
...
void SpdkEnv::DmaPoolPrewarm(size_t buf_size, int count, size_t align) {
    ...
    if (ok < count) {
        UMBP_LOG_ERROR("SpdkEnv: DMA pool pre-warm PARTIAL %d/%d × %zu bytes",
                       ok, count, buf_size);
    } else {
        UMBP_LOG_INFO("SpdkEnv: DMA pool pre-warmed %d × %zu bytes",
                      ok, buf_size);
    }
}
