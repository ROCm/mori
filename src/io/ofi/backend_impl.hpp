...
class OfiManager {
 ...
 private:
  ...
  void CloseAll();  // close all NIC fids + free info_ (dtor and ctor failure path)
...
// In-memory peer-address book for the OFI backend. Peer endpoint addresses
// (EngineDesc) and MR keys (MemoryDesc) travel inline in backendDescs, so there
// is no socket/thread — nothing is exchanged out of band. Responsibilities:
//   * SeedRemoteAddrs: on RegisterRemoteEngine, insert each of a peer's
//     advertised per-NIC addr_names into EVERY local NIC's AV (an fi_addr_t is
//     only valid for the AV it was inserted into), and cache the results.
//   * GetRemoteAddr: per transfer, return the fi_addr_t for sending from a given
//     local NIC to a specific (remote engine, remote NIC).
//   * RemoveRemoteAddrs: drop all cached addrs for a peer.
class OfiControlPlane {
 public:
  ...
  void RemoveRemoteAddrs(const EngineKey& key);
...
class OfiBackendSession : public BackendSession {
 public:
  ...
  OfiBackendSession(OfiManager* ofi,
