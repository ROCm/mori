...
#    apt-get install -y openmpi-bin libopenmpi-dev pybind11-dev
  ...
  cmake -DUSE_ROCM=ON -DCMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} \
      -DCMAKE_HIP_FLAGS=${CMAKE_HIP_FLAGS} -DBUILD_COLLECTIVE=ON \
      -DCMAKE_CXX_FLAGS="-DNDEBUG -Wno-deprecated-literal-operator" -DCMAKE_HIP_FLAGS="-DNDEBUG" \
      -DBUILD_EXAMPLES=ON -DWARP_ACCUM_UNROLL=1 -DUSE_BNXT=OFF \
      -DBUILD_APPLICATION=ON -DBUILD_SHMEM=ON -DBUILD_OPS=OFF -DBUILD_IO=OFF \
      -DBUILD_PYBINDS=OFF -DBUILD_UMBP=OFF -DBUILD_TESTS=ON -DENABLE_PROFILER=OFF \
      ...
      -DBUILD_TORCH_BOOTSTRAP=OFF -DBUILD_CCO=ON -DBUILD_CCO_SDMA=ON \
      -DGPU_TARGETS=gfx950 ..  
fi
