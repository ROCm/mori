...
#    apt-get install -y openmpi-bin libopenmpi-dev pybind11-dev
  ...
  rm -rf *
  cmake -DUSE_ROCM=ON -DCMAKE_BUILD_TYPE=Release -DBUILD_COLLECTIVE=ON \
  ...
  # cmake -DUSE_ROCM=ON -DCMAKE_BUILD_TYPE=RelWithDebInfo -DBUILD_COLLECTIVE=ON \
  #     -DBUILD_EXAMPLES=ON -DWARP_ACCUM_UNROLL=1 -DUSE_BNXT=OFF \
  #     -DBUILD_APPLICATION=ON -DBUILD_SHMEM=ON -DBUILD_OPS=OFF -DBUILD_IO=OFF \
  #     -DBUILD_PYBINDS=OFF -DBUILD_UMBP=OFF -DBUILD_TESTS=OFF -DENABLE_PROFILER=OFF \
  #     -DENABLE_DEBUG_PRINTF=OFF -DENABLE_STANDARD_MOE_ADAPT=OFF -DBUILD_OPS_DEVICE=OFF \
  #     -DBUILD_XLA_FFI_OPS=OFF -DWITH_MPI=ON -DMORI_MULTITHREAD_SUPPORT=ON \
  #     -DBUILD_TORCH_BOOTSTRAP=OFF \
  #     -DCMAKE_HIP_FLAGS="-gline-tables-only" \
  #     -DGPU_TARGETS=gfx950 .. 
fi
