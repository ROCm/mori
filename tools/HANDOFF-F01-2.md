...
### 20.11 基线搞错了一整轮:`MORI_COMB_PULL=off` 是三条线里最慢的那条

**`MORI_COMB_PULL` 不是 PUSH/PULL 传输开关**,是选谁 staging 调用方的输入(`host`/`kernel`/`off`)。
本会话所有实验的 `BASE` 都是 `off`,即 §18.2 表里 318.8 那条。省下的 8.5% 是真的,但在错的线上。

实测(HEAD `1e3729d`,64×8,EP4 bf16,**全部 rc=0**):

| 路线 | combine | GB/s |
|---|---|---|
| ZC=1 零拷贝 PULL | **167.6** | 1266.8 |
| ZC=0 host d2d + PULL(默认) | 233.9 | 907.7 |
| ZC=0 PUSH + FOLDB(本会话) | 288.2 | 736.6 |

PUSH 的直接对手是 **233.9**(同为 ZC=0),差 54.3;用户拿 **167.6** 当标杆,差 120.6。

### 20.12 但 PUSH 的传输比 PULL **快 20%**;它输在两段串行,不输在带宽

同一口径 212.3 MB:

```
PUSH 跨卡写   288.2 - 149.2 = 139.0us  ->  1527 GB/s
PULL 跨卡读            167.6us         ->  1267 GB/s
```

**PUSH 的跨卡写比 PULL 的跨卡读快 20%。**PUSH 慢是因为它多搬一趟本地读(fold 105.7)
**而且那趟和跨卡写完全不重叠** —— 中间是全局 barrier,严格 `push → barrier → fold`。

完全流水后的下限:

```
max(跨卡写 139.0, fold 105.7 + wait 20 + setup 10 = 135.7) ~= 139
+ 无法重叠的尾巴/barrier                                    ~= 150us
```

**即 PUSH 流水化后 ~150us,比 PULL 的 167.6 还快。**两段几乎等长(139 vs 135.7),
是近乎理想的流水形状 —— 这不是把 288 磨到 170,是把两段叠起来。

做法:拆掉全局 barrier,改成**按批**同步。token 分 N 批,批 k push 完只通知 peer 批 k 就绪,
peer fold 批 k 的同时本 rank push 批 k+1。要点:

- 同步粒度换成 per-(peer,批) 的 flag/counter,不能再用一次性全局 barrier(单次 15.6us,N 次会吃掉收益)
- 批数 N 要权衡:N 小则重叠不足,N 大则同步开销与 TDM descriptor 变碎
- **先做的不是写代码,是量"批级同步"的单价** —— 拿现有 barrier 原语测 N=2/4/8 的纯同步成本,
  低于 (139 - 150/N) 才有意义

优先级高于 §20.10 的双缓冲:双缓冲上界 30us,这个上界 120us,而且双缓冲是它的子集
(流水起来之后 cWait 自然被 push 掩盖)。
...
## 21. 2026-08-04 下午:combine 慢 18% 是**节点单 warp TDM 吞吐掉了**,与代码无关

一下午的产出主要是这条。**本节所有绝对值都是退化态下测的,不可与 §6/§14/§19/§20 的表比。**
等节点回到健康区后要重测的清单在 21.6。

### 21.1 现象:同一 commit,隔 1.5 小时慢 18%

```
                       12:53 左右        14:14 之后        差
1e3729d  ZC=1 combine   167.6            197.2           -17.6%
1e3729d  ZC=1 dispatch  173.6            207.7           -19.6%
```

`1e08978`(当前 HEAD)读 197.5/211.2,与 `1e3729d` 今天的数一致 ⇒ **FOLDB 和 `b45a9d99` 都没有
引入回归**。四次复测 197.5 / 198.2 / 197.2 / 198.2,±0.3%,不是抖动。

**回滚验证的坑**:`REV=1e3729d` 直接给裸 sha 会 `fatal: couldn't find remote ref`(容器是 shallow
single-branch clone),**HEAD 原地不动而脚本继续跑**,读出 197 会被当成"回滚后也慢"。已把该 sha 推成
分支 `perf-ref-1e3729d`,回滚一律走分支名,且核对 `HEAD=`。

已排除,每条都有实证:

- **邻居抢卡/抢 CPU**:`tiwang_dev` 400% CPU 那四个进程 `etime` 显示起于 07:08 UTC,比最后一次
  测量(06:47)晚 21 分钟。**我拿它解释过一次,错的。** `ps` 必须带 `etime`,只看 `%CPU` 会把二十分钟
  后才出生的进程当成原因。`_ct_gpustate.sh` / `_node_gpuusers.sh` 已加 `etime` 与各容器 CPU。
- **容器内残留进程持有资源**:29 个僵尸 python3 的 PPID 全是 1(`sleep infinity`),即原父进程早已
  退净、孤儿被 init 接管,而 `sleep infinity` 不 reap。**僵尸本身无副作用**(内核已回收内存/fd/KFD
  上下文,只剩 task_struct 和 PID),不占 GPU。`docker restart` 因此也没有诊断价值。
- **GPU 降频**:`sclk` 读数正常 —— 但按 §3 这读数本身不可信,所以这条**既未证实也未排除**。
- **`GPU%`=100 / 1130W / GPU2 比邻居高 20°C**:全是 §3 早就记过的冻结假读数。**我又照着它推了一轮
  "有残留 kernel 占 CU",白费。** 判节点只认 VRAM、`--showpids`、进程表、以及跑基准。

### 21.2 顺带做完:ZC=0 staging copy 实测 224 MiB,不是假设的 212 MB

`MORI_COMB_CPTIME=1` 打印该 copy 的尺寸(`dispatch_combine.py`)。**不能在这里计时**:bench 把
combine 抓进 cuda graph(`bench_dispatch_combine.py:180`),capture 期间 `synchronize` 会抛
`hipErrorStreamCaptureUnsupported` —— event 计时那版根本没跑到 kernel。时长仍用减法(copy 在 ZC=0
的 graph 里、ZC=1 的没有,两侧 kernel 逐位相同),尺寸靠打印,两者分开取。

```
[CPTIME] rows=16384 cols=7168 dtype=bf16 bytes=234881024 (224.00 MiB)
ZC=0 host 263.2us - ZC=1 197.5us = 65.7us   (历史 66.3 / 67.7,三次一致)
=> 有效载荷 3575 GB/s,HBM 上读+写 7150 GB/s
```

**16384 = 4096 tokens/rank × 4 rank,是接收缓冲满容量。** 一度以为其中一半是空行、可省 33us:
**错的**。行数由"该 source rank 上有多少 token 的 8 个 expert 里至少一个落在本 rank"决定,
`1 - C(192,8)/C(256,8) ≈ 0.903`,即每 source rank 发约 3701 行,四家合计约 14803 行,**满容量的
90%**。空行只有 ~10%(22.7 MB / 6.3us),"少搬一半"不成立,kernel staging 也就没有重测价值。
(0.903 是按均匀随机 topk 算的**推断**,bench 实际路由生成方式未读;真值有倾斜也只会在 90% 附近。)

结论:这 65.7us 在 MORI 内部无解。`get_registered_combine_input_buffer` 本就是公开 API
(`docs/MORI-EP-GUIDE.md:272-279`),caller 让 expert GEMM 直接输出到那块即可,copy 根本不发生。
它是 caller 写进自己 tensor 的代价,不是实现问题。

### 21.3 判据:epcheck 四项全过,**但它看不见这个故障**

```
a2a grid=512   1617.7  基线 1627.6  PASS      CU copy 1way   1640.8  基线 1641.0  PASS
TDM copy 1way  1620.5  基线 1637.3  PASS      TDM store 1way 1636.2  基线 1643.0  PASS
```

CU copy 是重复性 ±0.1% 的敏感项,分毫不差 ⇒ **链路和硬件没坏**。连 fork 四 rank 的 a2a 也正常,
所以"多 rank 同步退化"也不成立。原因是四项**全部落在带宽饱和段**(三项两进程点对点,a2a 跑
grid=512),而故障只在低并发暴露。**已在 `epcheck.sh` 追加 L4 低并发段**,见 21.6。

epsim mode0 才看得见。同口径(`GRIDS=64 BLOCK=256`,默认 NT)健康区 1545-1610,今天 **1309.4**。
`_lds_ok` 会拿 `RDELEMS/RDN` 挡住**根本不用它们的 mode0**(默认 7168:4 要 458752B),给一对小值即可
放行;已实测 **stride 对 mode0 无影响**(7168B→1308.2 / 14336B→1309.4),不必为此纠结口径。

### 21.4 定位:固定开销正常,**渐近带宽 -13%**,低并发把它放大到 19%

复现 §14.2 曲线(243 MB 工作集):

```
grid       32      64      96     128     256
历史     1346    1748    1773    1766    1604
今天    896.1  1408.5  1520.3  1545.7  1541.0
差       -33%    -19%    -14%   -12.5%   -3.9%
```

固定 grid=64 扫 op 尺寸(mode9,对照 §19.2):

```
 op 字节   历史 GB/s   今天 GB/s     差
   512        74.8       72.3      -3.3%   <- 噪声内(同参数两次跑 74.9/72.3,自身 ±3.5%)
  7168       895.6      879.2      -1.8%   <- 噪声内
 14336      1367.3     1212.5     -11.3%
 16384      1428.5     1257.7     -12.0%
 32768      1489.7     1295.6     -13.0%
```

小 op 是固定开销主导 ⇒ **每次发射的固定开销没变**;大 op 是带宽主导 ⇒ **单位时间搬的字节少了 13%**。
两个视角同指一处:**单个 warp 的 TDM 吞吐降约 13%**。warp 多时总量饱和在 fabric 上,缺口被并发填回
(grid=256 只差 3.9%,epcheck grid=512 直接 PASS);warp 少时无可填,直接暴露(grid=64 差 19%)。

**这正是 combine 所在的位置**:几何被钉在 64 block(其余 CU 属于要与之重叠的 GEMM,见 20.1 的更正),
它买不回并发,所以吃满这 18%。而单卡 `copy_` 走 DMA、epcheck 的 CU copy 走 CU,都不经这条路,
所以纹丝不动 —— 这也解释了为什么 65.7us 在退化态下毫无变化。

比值侧的旁证:`dispatch/epsim = 1027.8/1308.2 = 0.786`,§10 记的健康值是 0.77-0.79。
**内核相对当前天花板的效率完全正常**,掉的整个是天花板。

**成因未定。** 时钟这条排不掉也证不了(`fclk`/`sclk` 读数按 §3 不可用),再往下需要 fabric 计数器。
与 §10 的 2026-07-30 是同一类现象(当时 epsim -19.8% / dispatch -21.6%,起因同样未定,
首选假设"邻居竞争"—— 但今天机器实测空闲(load 0.23、各容器无活 python)照样退化,**竞争假设在今天
不成立**)。两次都没定因,说明这是个会反复出现的节点态,判据比归因重要。

### 21.5 处置

**不追这 13%**,它不是 mori 引入的也不是我们能修的。**绝对值一律不与历史表比,同时段跑一次
epsim 做分母、看比值**,A/B 依然有效。`_ct_bgwork.sh` 已在跑测前把环境快照(UTC、load、容器、
top 进程带 etime、VRAM)打进同一份日志 —— §20 每个数都是裸的,没有分母也没有环境,这是今天从
"以为是回归"查到"其实是天花板"花掉一下午的直接原因。

### 21.6 等新环境正常后要做(**本节数据都要重测**)

1. **给 `epcheck.sh` 的 L4 定标**。已追加低并发段(grid=64/128 + `sat64 = g64/g512`),
   **三个基线 `BL_A2A_G64` / `BL_A2A_G128` / `BL_SAT64` 故意留空**:今天的节点在退化态,记下来等于
   把故障固化成参考;而 epcheck 从来没跑过 grid=64,连历史值都没有。未填时报告并 skip,退出码含义
   不变;填上即自动成为 gate。
   **定标条件:该节点既通过 L0-L3,`epsim mode0 GRIDS=64 BLOCK=256` 又落在 1545-1610。两个都要
   —— L0-L3 通过恰恰是这次没抓到故障的原因。**
   `sat64` 阈值**不可从 epsim 搬**:今天同参数下 epcheck 1265.9 / epsim 1411.5 (grid=256)、
   1617.7 / 1372.7 (grid=512),两者差 11-18% 且大小关系相反,是不同 kernel。
2. **重测 21.1 / 21.4 全部**,确认健康态下 combine 回到 167.6 一线。
3. **PUSH 在健康态下的数**:§20.11 的 288.2 是健康态测的,可直接沿用;但要与 host+PULL 233.9
   背靠背再跑一次确认。
4. 跑一次 epcheck L4 拿健康态 grid=64/128 读数(退化态读数本轮未测,用户决定不在坏节点上采)。

### 21.7 工具

- **`tools/_ct_epchk.sh`(新)**:起临时容器跑 epcheck、跑完即删,不在节点上留 `EPCHECK-V1`。
  镜像本地已有 `mori-epcheck:gfx1250-20260731`,不必拉私有仓库。
  `--ipc=host` 和 `--shm-size 64g` 不能省(tdma2a fork 四 rank 走 IPC 句柄共享显存,
  ualoe_bw 两进程走 loopback 汇合),少了容器能起但用不了 GPU。
- **`_send_ct.ps1 -Envp` 的值里不能有 `|`**,会被当管道:`PAT=Error|Traceback` 直接报
  `command not found`。要么用单个词,要么 `PAT=.` 配 `NLINE=` 看尾部。
- **`_ct_epsim.sh` 的 `RDSWEEP="elems:n,elems:n"`** 一次 build 扫多个 op 尺寸,mode9 就是为拆
  "读尺寸 vs 在飞数量"这一对而存在的。
- **别一次 `AwaitShell` 睡 240s**:两档 EP 跑从启动到写完只要约 40 秒,10 秒是 bench 本身,外面是
  git fetch、四进程 spawn、分布式初始化和 graph capture。按 30-60 秒轮询。
