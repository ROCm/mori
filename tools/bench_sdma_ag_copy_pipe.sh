...
#   PIPELINE_CU=224 PIPELINE_CHUNKS=4 RUN_P2P_FUSED=1 \
...

{
  ...
  if [ "$RUN_PHASE_TIMING" = "1" ]; then
    ...
    if [ "$RUN_P2P_FUSED" = "1" ]; then
      run_phase_case "P2P_FUSED_PHASE_STAGE0" \
        MORI_CONTINUOUS_PREP=0 \
        MORI_P2P_FUSED=1 \
        MORI_PIPELINE_CU="$PIPELINE_CU" \
        MORI_PIPELINE_CHUNKS="$PIPELINE_CHUNKS" \
...
grep -E "MORI_P2P_FUSED|MORI_SDMA_AG_COPY_PIPE|Table 1:|copy vs RCCL|Copy-path|host-side hipMemcpy|gpu-side copy|AR\\[[0-3]\\] duration|median wall" "$LOG" || true
