...
: "${PIPELINE_CHUNKS:=4}"
: "${RUN_BASELINE:=1}"
: "${RUN_COPY_TO_USER_ONLY:=0}"
: "${RUN_RING_SHARD_ROUND_KERNELS:=0}"
: "${RING_SHARD_BLOCKS:=224}"
...

{
  ...
  if [ "$RUN_RING_SHARD_SDMA" = "1" ]; then
    if [ "$RUN_COPY_TO_USER_ONLY" = "1" ]; then
      run_copy_to_user_case "RING_SHARD_SDMA_COPY_TO_USER_ONLY" \
        MORI_CONTINUOUS_PREP=0 \
        MORI_RING_SHARD_DIRECT=1 \
        MORI_RING_SHARD_CU_DEBUG=0 \
        MORI_RING_SHARD_ROUND_KERNELS="$RUN_RING_SHARD_ROUND_KERNELS" \
        MORI_RING_SHARD_BLOCKS="$RING_SHARD_BLOCKS" \
        MORI_PIPELINE_CU="$PIPELINE_CU" \
        MORI_PIPELINE_CHUNKS="$PIPELINE_CHUNKS"
    else
  ...
  if [ "$RUN_RING_SHARD_PROBE" = "1" ]; then
    run_case "RING_SHARD_SDMA_PROBE" \
      MORI_CONTINUOUS_PREP=0 \
      MORI_RING_SHARD_DIRECT=1 \
  ...
  if [ "$RUN_RING_SHARD_DIRECT" = "1" ]; then
    run_case "RING_SHARD_CU_DEBUG" \
      MORI_CONTINUOUS_PREP=0 \
      MORI_RING_SHARD_DIRECT=1 \
      MORI_RING_SHARD_CU_DEBUG=1 \
      MORI_PIPELINE_CU="$PIPELINE_CU" \
      MORI_PIPELINE_CHUNKS="$PIPELINE_CHUNKS"
  fi
  ...
  if [ "$RUN_PHASE_TIMING" = "1" ]; then
    ...
    if [ "$RUN_RING_SHARD_DIRECT" = "1" ]; then
      run_phase_case "RING_SHARD_CU_DEBUG_PHASE_STAGE0" \
        MORI_CONTINUOUS_PREP=0 \
        MORI_RING_SHARD_DIRECT=1 \
        MORI_RING_SHARD_CU_DEBUG=1 \
        MORI_PIPELINE_CU="$PIPELINE_CU" \
...
grep -E "MORI_RING_SHARD_DIRECT|MORI_RING_EXECUTOR|MORI_CHUNKED_DIRECT|MORI_ONESHOT_DIRECT|MORI_SDMA_AG_COPY_PIPE|Table 1:|copy vs RCCL|Copy-path|host-side hipMemcpy|gpu-side copy|AR\\[[0-3]\\] duration|median wall" "$LOG" || true
