...
// Fixed-size MPI slot for a msgpack-serialized descriptor. The OFI backend
// carries its addr-names (CXI ≤ 64 B each) and MR keys inline in backendDescs,
// so the descriptors must be exchanged in full, not as a few scalar fields.
static constexpr size_t kDescSlot = 8192;  // [uint32 len][msgpack bytes]

template <class T>
static std::array<uint8_t, kDescSlot> PackDesc(const T& v) {
  msgpack::sbuffer sb;
  msgpack::pack(sb, v);
  if (sb.size() + sizeof(uint32_t) > kDescSlot) die("descriptor exceeds slot size");
  std::array<uint8_t, kDescSlot> slot{};
  uint32_t len = static_cast<uint32_t>(sb.size());
  std::memcpy(slot.data(), &len, sizeof(len));
  std::memcpy(slot.data() + sizeof(len), sb.data(), sb.size());
  return slot;
}

template <class T>
static T UnpackDesc(const uint8_t* slot) {
  uint32_t len = 0;
  std::memcpy(&len, slot, sizeof(len));
  auto oh = msgpack::unpack(reinterpret_cast<const char*>(slot + sizeof(len)), len);
  return oh.get().as<T>();
}
...
static int RunTest(int rank, int nranks) {
  ...
  // ── Exchange EngineDescs (full msgpack, incl. inline OFI addr-names) ───────
  auto myDesc = engine.GetEngineDesc();
  auto myDescSlot = PackDesc(myDesc);
  std::vector<uint8_t> allDescs(static_cast<size_t>(nranks) * kDescSlot);
  MPI_Allgather(myDescSlot.data(), kDescSlot, MPI_BYTE,
                allDescs.data(), kDescSlot, MPI_BYTE, MPI_COMM_WORLD);
  for (int i = 0; i < nranks; i++) {
    if (i == rank) continue;
    EngineDesc r = UnpackDesc<EngineDesc>(allDescs.data() + static_cast<size_t>(i) * kDescSlot);
  ...
  // ── Exchange MemoryDescs (full msgpack, incl. inline OFI MR keys) ─────────
  // Two descriptors per rank, laid out as [send | recv].
  std::array<uint8_t, 2 * kDescSlot> myMemSlots{};
  std::memcpy(myMemSlots.data(),             PackDesc(sendMem).data(), kDescSlot);
  std::memcpy(myMemSlots.data() + kDescSlot, PackDesc(recvMem).data(), kDescSlot);
  std::vector<uint8_t> allMemSlots(static_cast<size_t>(nranks) * 2 * kDescSlot);
  MPI_Allgather(myMemSlots.data(), 2 * kDescSlot, MPI_BYTE,
                allMemSlots.data(), 2 * kDescSlot, MPI_BYTE, MPI_COMM_WORLD);
  std::vector<MemoryDesc> allSend(nranks), allRecv(nranks);
  for (int i = 0; i < nranks; i++) {
    const uint8_t* base = allMemSlots.data() + static_cast<size_t>(i) * 2 * kDescSlot;
    allSend[i] = UnpackDesc<MemoryDesc>(base);
    allRecv[i] = UnpackDesc<MemoryDesc>(base + kDescSlot);
  }

  int right = (rank + 1) % nranks;          // where we write to
  int left  = (rank + nranks - 1) % nranks; // who writes to us

  // Full remote descriptor — carries the inline OFI MR key via backendDescs.
  MemoryDesc rightRecvMem = allRecv[right];
  ...
  if (rank == 0) {
    MemoryDesc rank1SendMem = allSend[1];  // full desc, incl. inline OFI MR key
