...
# Reuse the benchmark's dist+shmem init pattern (registers "default" group).
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from utils import TorchDistContext, get_free_port  # noqa: E402
...
def _worker(rank, world_size, port):
    with TorchDistContext(rank=rank, world_size=world_size, master_port=port):
        ...
        for size_mb in [1, 16, 64, 128, 256]:
            ...
            if rank == 0:
                print(f"[size={size_mb:3d} MB] MISS ok={ok}  "
            ...
            if rank == 0:
                print(f"[size={size_mb:3d} MB] HIT  ok={ok}  "
