    ...
    with TorchDistContext(rank=rank, world_size=world_size, master_port=port):
        ...
        try:
            for dtype in _DTYPES:
                ...
                out_list = [torch.empty(numel, dtype=dtype, device=device)
                            for _ in range(world_size)]
                ref_list = [torch.empty(numel, dtype=dtype, device=device)
                            for _ in range(world_size)]
                dist.all_gather(ref_list, inp)
                assert handle.all_gather(out_list, inp,
...
def test_hier_allgather_list(world_size=None, ranks_per_node=None, numel=4096):
    ...
    torch.multiprocessing.spawn(
        _worker, args=(world_size, ranks_per_node, port, numel),
