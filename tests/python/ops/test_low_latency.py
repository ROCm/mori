...
def test_loop(local_rank: int, num_local_ranks: int):
    ...
    try:
        ...
        test_main(num_tokens, hidden, num_experts, num_topk, rank, num_ranks, group, None, seed=1)

        do_pressure_test = False
        for seed in range(int(1e9) if do_pressure_test else 0):
            if local_rank == 0:
                print(f'Testing with seed {seed} ...', flush=True)
            ref_hash = test_main(num_tokens, hidden, num_experts, num_topk, rank, num_ranks, group, None, seed=seed)
            for i in range(20):
                assert test_main(num_tokens, hidden, num_experts, num_topk, rank, num_ranks, group, None, seed=seed) == ref_hash, f'Error: seed={seed}'
