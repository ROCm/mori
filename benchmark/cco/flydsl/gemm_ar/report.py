...
MEASURED = {
    "hot_us", "cold_us", "us", "max_rank_time_us", "rel_l2", "validated",
    "copies", "cold_reps", "working_set_mb", "vram_before", "vram_after",
    "error", "timing", "route", "supported", "reason",
...
def main() -> int:
    ...
    p.add_argument("--col", default="cold_us",
                   help="which timing field to table (default cold_us)")
    ...
    if bad and not args.keep_invalid:
        ...
        print(f"!! {len(bad)} row(s) failed validation, excluded "
              f"(--keep-invalid to see them)")
        for r in bad[:5]:
            who = r.get("impl") or r.get("mode") or "?"
            print(f"     {r.get('shape','?')} M={r.get('m','?')} {who}: "
    ...
    def keyof(r):
        ...
        if axes is not None:
            ...
            keep += [k for k in ("label", "shape", "n", "k", "world_size",
                                 "quant", "scope") if k in r]
            return tuple((k, r[k]) for k in sorted(set(keep)))
        return tuple(
            (k, r[k]) for k in sorted(r)
            if k not in MEASURED and k not in (col_key, "m", "bench", "sweep_axes")
    ...
    for key, cells in sorted(table.items(), key=lambda kv: str(kv[0])):
        ...
        rest = "  ".join(f"{k}={v}" for k, v in key
                         if k not in ROW_KEYS and v not in (None, ""))
        ...
        print(f"{'M':>8}" + "".join(f"{str(c):>16}" for c in cols)
