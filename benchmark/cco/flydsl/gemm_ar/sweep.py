...
MAIN = ["wq_b", "wo_b"]
ALL_SHAPES = MAIN + [
    "wq_a_tp4", "wkv_tp4", "wqkv_a_tp4", "wo_a_tp4", "shared_gate_up_tp4",
    "wq_b_tp8", "wo_b_tp8", "wo_a_tp8", "wq_b_tp1", "wo_b_tp1",
]
...
PRESETS = {
    ...
    "fused-fp8": dict(
        ...
        args=["-n", "5120", "-k", "2048", "--quant", "mxfp8",
              "--gather-dtype", "fp8", "--gather-transport", "lsa",
              "--no-fuse-quantize"],
        label="mxfp8-fp8wire",
    ),
    "fused-wire": dict(
...
def require_sdma() -> None:
    ...
    if not BUILD_CCO_SDMA:
        sys.exit("BUILD_CCO_SDMA is OFF -- the SDMA path is compiled out, "
...
def occupancy() -> str:
    try:
        ...
        return "?"
    gib = [f"{int(p[2]) / 2**30:.0f}" for p in
...
def point_argv(preset, point, out_path):
    ...
    for key, val in point.items():
        if isinstance(val, bool):
            ...
            cmd.append(f"--{key.replace('_', '-')}" if val
...
def main() -> int:
    ...
    if args.dry_run:
        for point in preset["grid"]:
            print(" ".join(shlex.quote(c)
                           for c in point_argv(preset, point, out_path)))
        return 0
    ...
    for i, point in enumerate(preset["grid"], 1):
        ...
        if not ok:
            failed.append(desc)
            print(f"  FAILED: {out.strip().splitlines()[-1] if out.strip() else '?'}",
                  flush=True)
        else:
    ...
    if before != after:
        print("# NOTE: occupancy moved across the sweep; a leftover process may "
              "have shared the GPU. Re-run before trusting these.", file=sys.stderr)
    if failed:
