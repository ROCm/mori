...
//   C1: EpCombineLowLatencyAsyncSendCopy              → FFN output 拷到 staging (用 dispReceiverIdxMap 还原 slot)
//   C2: EpCombineLowLatencyAsyncSendTransfer          → IBGDA non-blocking put 回 src PE
//
...
int main(int argc, char** argv) {
  ...
  // Scoped so handle destructs (calls ShmemFree) before ShmemFinalize below.
  {
    ...
    handle.PrepareInference(HIP_R_16BF, d_ffn_out,
                            /*output=*/nullptr,
                            reinterpret_cast<float*>(d_weights),
                            reinterpret_cast<int32_t*>(d_indices), numTokens);
