...
def _make_put_kernel(coop, thread_mode, signal_op):
    ...
    gate_single = coop == cco.CoopScope.THREAD and thread_mode == cco.ThreadMode.INDEPENDENT

    @flyc.kernel(known_block_size=[THREADS, 1, 1])
    def put_kernel(dev_comm: Int64, send_win: Int64, recv_win: Int64, nbytes: Int64,
                   sig_id: Int32, sig_val: Int64):
        gda = cco.DevComm(dev_comm).gda(0)
        if gate_single:
            if fx.thread_idx.x == 0:
                gda.put(DST_RANK, recv_win, 0, send_win, 0, nbytes,
                        signal_op=signal_op, signal_id=sig_id, signal_val=sig_val,
                        coop=coop, thread_mode=thread_mode)
        else:
            gda.put(DST_RANK, recv_win, 0, send_win, 0, nbytes,
                    signal_op=signal_op, signal_id=sig_id, signal_val=sig_val,
                    coop=coop, thread_mode=thread_mode)
    ...
    def run(dev_comm: Int64, send_win: Int64, recv_win: Int64, nbytes: Int64,
            sig_id: Int32, sig_val: Int64, stream=fx.Stream(None)):
    ...
    with Communicator.init(nranks, rank, uid, per_rank_vmm=PER_RANK_VMM) as comm:
        ...
        for mname, coop, tm in MODES:
            for sname, sig_op, least in SIGNALS:
                ...
                if rank == 0:
                    put_runners[(mname, sname)](dc.ptr, send_win.handle, recv_win.handle,
                                                NBYTES, sig_id, ADD_VAL)
                else:
                ...
                if rank == DST_RANK:
                    ...
                    print(f"[combo mode={mname:12} signal={sname}] "
