...
__global__ void SdmaSelfCopyKernel(
  ...
  if (threadLinearId < numQueues) {
    ...
    size_t copyBytes = (qId == (int)(numQueues - 1)) ? (totalBytes - (numQueues - 1) * chunkBase) : chunkBase;
  ...
  if (threadLinearId < numQueues) {
    int qId = threadLinearId;
    HSAuint64* signal = srcObj->signalPtrs + static_cast<size_t>(myPe) * numQueues;
    HSAuint64 expected = srcObj->expectSignalsPtr[myPe * numQueues + qId] + 1;
    anvil::waitForSignal(signal + qId, expected);
    srcObj->expectSignalsPtr[myPe * numQueues + qId] = expected;
  }
  __syncthreads();
}
