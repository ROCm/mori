...
__global__ void RingAllGatherWithPutMemNbiThread(int myPe, int npes, SymmMemObj* memObj) {
...
void RingAllGatherWithPutMemAPI() {
  ...
  // Run put
  RingAllGatherWithPutMemNbiThread<<<1, 1>>>(myPe, npes, buffObj.gpu);
  HIP_RUNTIME_CHECK(hipDeviceSynchronize());
...
int main(int argc, char* argv[]) { RingAllGatherWithPutMemAPI(); }