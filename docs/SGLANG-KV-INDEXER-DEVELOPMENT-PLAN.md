...
- `ReportExternalKvBlocks`
- `RevokeExternalKvBlocks`
- `RevokeAllExternalKvBlocksAtTier`
- `MatchExternalKv`
- `GetExternalKvHitCounts`

- 定义 `TierType`、各 RPC request / response、`ExternalKvNodeMatch`、`TierHashes` 和 `HitCountEntry`。
...
BlockStored(block_hashes, medium)
  -> ReportExternalKvBlocks(worker_id, hashes, tier)

BlockRemoved(block_hashes, medium)
  -> RevokeExternalKvBlocks(worker_id, hashes, tier)

AllBlocksCleared()
  -> RevokeAllExternalKvBlocksAtTier(worker_id, tier)
```
