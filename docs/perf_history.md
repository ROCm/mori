...
## Entry 42 — Implement `MORI_GROUPED_REDUCE_BARRIER=<N>` (group-size 2 target)
- **Date**: 2026-04-30
- **Commit**: _this commit_
- **Motivation**: User asked to try the middle ground between per-chunk barrier
  and failed batch-all-chunks barrier. Entry 36 batch-all (`group=numChunks`)
  destroyed pipeline behavior; per-chunk barrier/control still contributes to
  AR service time. Group size 2 may reduce barrier count while preserving some
  reduce/AG overlap across groups.

### Implementation

Add env:
```bash
MORI_GROUPED_REDUCE_BARRIER=2
```

Only affects baseline multi-chunk `PipelinedAllReduceSdmaKernel`; default path
unchanged. For chunks=4 and group=2:
```text
wait reduce(c0,c1)
one cross-PE reduce_complete barrier (+2)
submit AG c0,c1
wait reduce(c2,c3)
one cross-PE reduce_complete barrier (+2)
submit AG c2,c3
wait all AG completions
```

This removes half of the reduce_complete barriers without fully batching all
AG after all reduce.
...
```bash
MORI_CONTINUOUS_PREP=0 MORI_PIPELINE_CU=224 MORI_PIPELINE_CHUNKS=4 \
MORI_GROUPED_REDUCE_BARRIER=2 python3 tests/python/ccl/test_allreduce.py \
  --num-stages 4 --elems 67108864 --iterations 1 --warmup 1 \
  --continuous-iters 100 --continuous-phase-iter 5 --continuous-phase-stage 0
```

Compare against Entry 34 best (`6.390 ms`) and Entry 36 batch-all failure
(`49.456 ms`).
