...
| RPC | 用途 |
|---|---|
| `ApplyExternalKvBatch`            | **主路径**:批量应用一批 store/remove/clear 动作 |
| `ReportExternalKvBlocks`          | (legacy) 上报某 worker 的 block |
| `RevokeExternalKvBlocks`          | (legacy) 撤销某 worker 的 block |
| `RevokeAllExternalKvBlocksAtTier` | 撤销某 tier 的全部 block(如整层清空) |
