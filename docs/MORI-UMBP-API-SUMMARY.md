...
| `dram` | DRAM tier 配置 |
| `ssd` | SSD tier 配置 |
| `eviction` | eviction 策略配置 |
| `copy_pipeline` | DRAM/HBM 到 SSD copy pipeline 配置 |
| `role` | `Standalone` / `SharedSSDLeader` / `SharedSSDFollower` |
| `distributed` | 可选 distributed 配置；为空表示 local-only |
...
dist.master_config.master_address = "10.0.0.1:15558"
dist.master_config.node_id = "node-a"
dist.master_config.node_address = "10.0.0.2"
dist.io_engine.host = "127.0.0.1"
dist.io_engine.port = 16000
dist.peer_service_port = 17000
...
| 字段 | 说明 |
| --- | --- |
| `capacity_bytes` | DRAM tier 容量 |
| `use_shared_memory` | 是否使用共享内存 |
| `shm_name` | shared memory 名称 |
| `high_watermark` / `low_watermark` | DRAM eviction 水位线 |
| `use_hugepages` | 是否使用 hugepage |
| `hugepage_size` | hugepage 大小 |
| `numa_node` | NUMA 绑定，`-1` 表示不绑定 |
| `prefault` | 是否预触页 |
...
| 字段 | 说明 |
| --- | --- |
| `enabled` | 是否启用 SSD tier |
| `storage_dir` | POSIX/file backend 目录 |
| `capacity_bytes` | SSD tier 容量 |
| `layout_mode` | 当前主要是 `SegmentedLog` |
| `segment_size_bytes` | segment 大小 |
| `high_watermark` / `low_watermark` | SSD 本地 eviction 水位线 |
| `io` | I/O backend 和 queue depth |
| `durability` | 持久化模式 |
| `ssd_backend` | `file` / `spdk` / `spdk_proxy` |
| `spdk_*` | SPDK 或 SPDK proxy 相关配置 |

### 8.4 `UMBPDistributedConfig`

| 字段 | 说明 |
| --- | --- |
| `master_config` | master 地址、node id、node address、heartbeat 等 |
| `io_engine` | MORI-IO engine host/port |
| `staging_buffer_size` | 通用 staging buffer 大小 |
| `ssd_staging_buffer_size` | remote SSD read staging buffer 总大小 |
| `ssd_staging_buffer_slots` | remote SSD read staging slot 数量 |
| `peer_service_port` | peer service 端口 |
| `cache_remote_fetches` | 是否缓存远程 fetch 回来的 block |
| `dram_page_size` | DRAM/HBM page size；`0` 表示使用 master 默认值 |
