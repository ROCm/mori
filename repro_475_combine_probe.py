...
def build_config(kernel, rank, world_size, hidden, topk, experts_per_rank,
                 cap, gpu_per_node, blocks):
    ...
    return mori.ops.EpDispatchCombineConfig(**{k: v for k, v in kwargs.items()
                                               if k in fields})


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--tokens", default="16 32 128 256 512",
                    help="tokens per rank to sweep")
    ...
    if rank == 0:
        ...
        print(f"[repro] world={world_size} gpu_per_node={args.gpu_per_node} "
              f"arch={props.gcnArchName} torch={torch.__version__}", flush=True)

    torch._C._distributed_c10d._register_process_group(
        "default", torch.distributed.group.WORLD)
    mori.shmem.shmem_torch_process_group_init("default")

    config = build_config(args.kernel, rank, world_size, args.hidden, args.topk,
                          experts_per_rank, cap, args.gpu_per_node,
                          (block_num, rdma_block_num, dwarps, cwarps))
    os.environ["MORI_EP_LAUNCH_CONFIG_MODE"] = "MANUAL"
    op = mori.ops.EpDispatchCombineOp(config)
    if rank == 0:
        print(f"[repro] kernel={args.kernel} blocks={args.blocks} cap={cap} "
              f"qp_per_pe={int(mori.shmem.shmem_num_qp_per_pe())}", flush=True)

    any_bad = 0
    for T in ladder:
        g = torch.Generator().manual_seed(args.seed * 100003 + T * 101 + rank)
        idx = torch.rand((T, args.experts), generator=g).topk(
            args.topk, dim=1).indices.to(torch.int32).to(device)
        ...
        out, _ow, _sc, out_idx, recv_num = op.dispatch(
            x, w, scales, idx, block_num=block_num,
            rdma_block_num=rdma_block_num, warp_per_block=dwarps)
        rows = int(recv_num[0].item())

        combined = op.combine(out, None, out_idx, block_num=block_num,
                              rdma_block_num=rdma_block_num, warp_per_block=cwarps)
        ...
        print(f"[repro][rank {rank}] T={T}: recv={rows} expected_recv={expected_recv} "
        ...
        if rank == 0:
            v = int(tot.item())
            print(f"[repro] T={T} GLOBAL: {'CORRUPTION' if v else 'clean'} "
                  f"({v} wrong tokens across {world_size} ranks)", flush=True)
        dist.barrier()
    ...
    if rank == 0:
        print(f"[repro] RESULT: {'FAIL (combine corruption)' if flag.item() else 'PASS'}",
              flush=True)
