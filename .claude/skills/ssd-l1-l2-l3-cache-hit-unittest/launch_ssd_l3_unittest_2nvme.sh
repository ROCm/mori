#!/bin/bash
# L1/L2/L3 cache-tier isolation unit test: pure TP8 (no DP-attention, no EP),
# single node, non-PD, 2-NVMe L3. max-total-tokens and hicache-ratio sized to
# barely hold request A (4096 tokens) alone in L1/L2, so a much larger request
# B (65536 tokens) unconditionally evicts A from both tiers while L3 (already
# holding A via write-through) stays untouched. max-running-requests=1
# serializes A/B/A with zero concurrent interference.
set -x
...
python3 -m sglang.launch_server \
  ...
  --hicache-storage-backend-extra-config '{"dram_capacity_bytes": 0, "ssd_enabled": true, "ssd_storage_dir": "/umbp_ssd/drive1,/umbp_ssd/drive2", "ssd_capacity_bytes": 68719476736, "ssd_backend": "file", "ssd_io_backend": "io_uring", "master_address": "'${UMBP_MASTER}'", "node_address": "'${NODE_IP}'", "io_engine_port": "19600", "peer_service_port": "19700", "cache_remote_fetches": false, "kv_events_subscriber": true, "kv_events_endpoint": "tcp://localhost:6557", "staging_buffer_size": 2147483648, "ssd_staging_buffer_slots": 512, "ssd_staging_buffer_size": 4294967296, "ssd_write_staging_slots": 512, "ssd_write_staging_size": 4294967296, "ssd_staging_use_hugepages": true, "ssd_staging_hugepage_size": 2097152}' \
